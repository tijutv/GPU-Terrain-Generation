...
void main(void)
{
	...
	//
	//float compareVal = fs_worldCoord.y/10.0;

	//vec4 mix0 = mix(color0, color1, smoothstep(0.0, 0.01, compareVal));
	//vec4 mix1 = mix(color1, color2, smoothstep(0.01, 0.2, compareVal));
	//vec4 mix2 = mix(color2, color3, smoothstep(0.2, 0.5, compareVal));
	//vec4 mix3 = mix(color3, color4, smoothstep(0.5, 0.8, compareVal));
	//vec4 mix4 = color4;

	//out_Color = mix0 * step(0.0, compareVal)  * (1.0-step(0.01, compareVal)) + 
	//	        mix1 * step(0.01, compareVal) * (1.0-step(0.2, compareVal))  +
